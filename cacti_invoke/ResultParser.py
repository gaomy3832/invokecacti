...
_RESULT_KEY2PATTERN = OrderedDict([
    ('latency', (r'Access time \(ns\)\s*:\s*({})'\
            .format(FLOAT_REGEX), \
            lambda x: float(x) * 1e-9)),
    ('eread', (r'Total dynamic read energy per access \(nJ\)\s*:\s*({})'\
            .format(FLOAT_REGEX), \
            lambda x: float(x) * 1e-9)),
    ...
    ('width', (r'Cache height x width \(mm\)\s*:\s*{0}\s*x\s*({0})'\
            .format(FLOAT_REGEX), \
            lambda x: float(x) * 1e-3)),
    ('area', (r'Cache height x width \(mm\)\s*:\s*({0}\s*x\s*{0})'\
            .format(FLOAT_REGEX), \
...
def parse(string):
    '''
    Parse the CACTI output string and return a dict containing the results.
    '''
